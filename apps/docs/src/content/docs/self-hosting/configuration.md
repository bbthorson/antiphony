...
| Variable | Purpose |
|---|---|
| `ANTIPHONY_APP_WEBHOOK_URLS` | Comma-separated `appId:url` pairs — where to POST each tenant's stage-settled events, e.g. `vox-pop:https://bff.voxpop/hooks`. Split on the first colon, so a URL with a port is fine. Must be **https** unless the host is loopback (`localhost`, `127.0.0.1`, `::1`), which stays plaintext-friendly for developing a receiver locally. |
