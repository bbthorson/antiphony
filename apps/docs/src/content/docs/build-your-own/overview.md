...
| **Capture primitives** | `packages/capture-kit` (`@antiphony/capture-kit`) | Headless React hooks for the browser side: a mic recorder, a playback hook, and the waveform helper that produces lexicon-shaped peaks. Renders nothing, styles nothing, uploads nothing. See [the capture kit](/build-your-own/capture-kit/). |
