...
/**
 * Storage arrives as a parameter rather than an import so the blob backend can
 * be chosen independently of the database — a deployment on Neon with blobs
 * still in GCS is a valid intermediate state during the migration, and an
 * import would have hard-wired the pairing.
 */
export function firebaseAudioProcessingDependencies(
