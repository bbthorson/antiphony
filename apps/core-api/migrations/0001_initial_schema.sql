-- Antiphony on Neon — 0001, the initial schema
--
-- ⚠️ THIS FILE IS FROZEN. IT HAS BEEN APPLIED. DO NOT EDIT IT.
--
-- `scripts/migrate.ts` records a CHECKSUM of every file it applies and refuses
-- to run at all if one changes afterwards. That is deliberate — it turns "I
-- tweaked 0001 and it didn't take" from a mystery into a message — and it means
-- an edit here is a hard error, not a no-op. Schema changes go in a NEW
-- numbered file beside this one.
--
-- This was `db/schema.sql`, applied by hand with `psql -f`. It is unchanged
-- except that its own `begin;` / `commit;` are gone: the migrator wraps each
-- file in its own transaction, and a file that commits halfway through would
-- close that transaction early and leave the rest unprotected.
--
-- The guidance that used to head this file — the design rules, the Postgres 18
-- `stored` warning, the sweep-scheduling notes and the open questions — moved
-- to `../db/README.md`, because all of it is advice to whoever writes the NEXT
-- migration and none of it can live in a file nobody may edit.
--
...
-- Drives the TTL sweep (see ../db/README.md § Open questions).
